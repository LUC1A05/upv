...
public class Main {
	...
	private int moves(Cell c1, Cell c2) { 
		...
		Queue<Cell> aztGabe = new LinkedList<>();
		boolean[][]aztertuak = new boolean[ROWS][COLUMNS];
		int[][]movs = new int[ROWS][COLUMNS];
		
		int[]x = {-2,-2,-1,-1,1,1,2,2};
		int[]y = {1,-1,2,-2,2,-2,1,-1};
		
		aztGabe.add(inicio);
		aztertuak[inicio.x][inicio.y] = true;
		movs[inicio.x][inicio.y] = 0;
		while(!aztGabe.isEmpty()) {
			Cell unekoa = aztGabe.remove();
			for(int i = 0; i < x.length; i++) {
				if(unekoa.x + x[i] < 8 && unekoa.y + y[i] < 8 && unekoa.x + x[i] >= 0 && unekoa.y + y[i] >= 0) {
					if(!aztertuak[unekoa.x + x[i]][unekoa.y + y[i]]) {
						aztertuak[unekoa.x + x[i]][unekoa.y + y[i]] = true;
						aztGabe.add(new Cell(unekoa.x + x[i], unekoa.y + y[i]));
						movs[unekoa.x + x[i]][unekoa.y + y[i]] = movs[unekoa.x][unekoa.y] +1;
					}
				}
			}
		}
